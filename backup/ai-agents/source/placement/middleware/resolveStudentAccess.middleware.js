...
/**
 * Resolves req.placementActor = { userId, role, studentId } for every
 * placement route. Same pattern as every other student-scoped agent's
...
const resolveStudentAccess = async (req, res, next) => {
  try {
    ...
    if (!userId || role === "GUEST") {
      throw new ApiError(401, "Authentication required to access placement data");
    }
    ...
    req.placementActor = { userId, role, studentId };
    next();
  } catch (error) {
    next(error);
  }
