 ...
 * correctAnswer that is one of those options verbatim — see
 * question.helper.js's normalizeCorrectAnswer) are eligible.
 *
...
const buildFallbackQuestionSet = async (courseId, concepts) => {
  ...
  const resolveConceptForRow = (row) => {
    ...
    return null; // no confident match — question is still usable, just not tied to a module for time-personalization
  };
