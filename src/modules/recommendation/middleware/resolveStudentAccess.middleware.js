...
/**
 * Resolves req.recommendationActor = { userId, role, studentId } for every
 * recommendation route. Same pattern as every other agent's resolve*Context
...
const resolveStudentAccess = async (req, res, next) => {
  try {
    ...
    if (!userId || role === "GUEST") {
      throw new ApiError(401, "Authentication required to access recommendation data");
    }
    ...
    req.recommendationActor = { userId, role, studentId };
    next();
  } catch (error) {
    next(error);
  }
