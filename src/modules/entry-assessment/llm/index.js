 ...
 * in the course's quizzes. If neither path can produce a credible set,
 * returns an empty list with a `failureReason` — never fabricated questions
 * with invented correct answers.
 *
...
const generateQuestions = async ({ course, concepts, profileSummary }) => {
  if (anthropicProvider.isConfigured()) {
    try {
      ...
      console.error("[entry-assessment] LLM question generation failed, falling back to real question bank:", error.message);
    }
  }
