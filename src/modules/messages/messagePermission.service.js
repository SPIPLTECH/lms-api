...
/**
 * Check whether a user can start a direct conversation
 * Business Rules:
 *
 * 1. Admin -> Anyone
...
const canMessage = async (senderId, receiverId) => {
    ...
    if (
        ...
        if (!enrollment) {
            throw new Error(
                "You can only message instructors of your enrolled courses."
            );
        }
    ...
    if (
        ...
        if (!commonCourse) {
            throw new Error(
                "Students can message only classmates enrolled in the same course."
            );
        }
    ...
    if (
        ...
        throw new Error(
            "Instructors are not allowed to message other instructors."
        );
    }
    ...
    throw new Error("You are not authorized to message this user.");
};
