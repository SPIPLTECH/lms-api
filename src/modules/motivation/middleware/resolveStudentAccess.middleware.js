...
/**
 * Resolves req.motivationActor = { userId, role, studentId } for every
 * motivation route. Same pattern as every other agent's resolve*Context
...
const resolveStudentAccess = async (req, res, next) => {
  try {
    ...
    if (!userId || role === "GUEST") {
      throw new ApiError(401, "Authentication required to access motivation data");
    }
    ...
    req.motivationActor = { userId, role, studentId };
    next();
  } catch (error) {
    next(error);
  }
