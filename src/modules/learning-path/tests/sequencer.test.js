...
const courseStructure = [
  {
    id: "m1",
    lessons: [
      { id: "l1", title: "Intro", contents: [{ duration: 600 }] }, // 10 min
      { id: "l2", title: "Basics", contents: [] }, // default
    ],
  },
  {
    id: "m2",
    lessons: [{ id: "l3", title: "Advanced", contents: [{ duration: 300 }, { duration: 300 }] }], // 10 min
  },
];
...
test("estimateLessonMinutes sums real Content.duration, falls back to the default when none exists", () => {
  assert.equal(estimateLessonMinutes({ contents: [{ duration: 600 }] }), 10);
  assert.equal(estimateLessonMinutes({ contents: [] }), 20);
  assert.equal(estimateLessonMinutes({ contents: null }), 20);
});
