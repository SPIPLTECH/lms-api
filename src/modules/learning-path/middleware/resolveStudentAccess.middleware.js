...
/**
 * Resolves req.learningPathActor = { userId, role, studentId } for every
 * learning-path route. Same pattern as every other student-scoped agent's
...
const resolveStudentAccess = async (req, res, next) => {
  try {
    ...
    if (!userId || role === "GUEST") {
      throw new ApiError(401, "Authentication required to access learning path data");
    }
    ...
    req.learningPathActor = { userId, role, studentId };
    next();
  } catch (error) {
    next(error);
  }
