...
test("resolveTargetStudentId locks a STUDENT actor to their own id when none requested", () => {
  assert.equal(resolveTargetStudentId({ role: "STUDENT", studentId: "s1" }, undefined), "s1");
});

test("resolveTargetStudentId allows a STUDENT actor to explicitly request their own id", () => {
  assert.equal(resolveTargetStudentId({ role: "STUDENT", studentId: "s1" }, "s1"), "s1");
});
...
test("resolveTargetStudentId requires staff to name a target", () => {
  assert.throws(() => resolveTargetStudentId({ role: "INSTRUCTOR" }, undefined), ApiError);
...
test("resolveTargetStudentId allows staff to name any target", () => {
  assert.equal(resolveTargetStudentId({ role: "ADMIN" }, "s9"), "s9");
