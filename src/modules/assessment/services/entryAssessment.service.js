...
/**
 * Generate -> Evaluate -> Initialize Student State pipeline. Idempotent: a
 ...
 * this student's per-course baseline in the Student State Agent — the
 * hand-off the spec's architecture diagram describes (Assessment Agent ->
 * Student State Agent -> Learning Path Agent). The initializeCourseState
 * call is awaited so the dashboard has fresh data the moment this request
 * returns, but its failure doesn't invalidate an already-successful
 * evaluation — it's logged and surfaced via `stateInitialized`.
 *
 * @param {string} studentId
 * @param {string} courseId
...
const submitEntryAssessment = async (studentId, courseId, answers) => {
  ...
  try {
    await studentState.initializeCourseState(studentId, courseId, evaluation);
    stateInitialized = true;
  } catch (error) {
    console.error(`[assessment:entry] failed to initialize student-state for student ${studentId}, course ${courseId}:`, error);
  }

  assessmentBus.publish(ASSESSMENT_EVENT_NAMES.ENTRY_ASSESSMENT_EVALUATED, {
    studentId,
    courseId,
