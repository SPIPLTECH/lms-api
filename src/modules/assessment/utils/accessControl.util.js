...
/**
 * A STUDENT actor may only ever resolve to their own studentId; staff
 * must explicitly name a target. Same rule as every other agent in this
 * codebase — assessment data is just as sensitive as activity/state data.
 *
...
const resolveTargetStudentId = (actor, requestedStudentId) => {
  if (actor.role === "STUDENT") {
    if (requestedStudentId && requestedStudentId !== actor.studentId) {
      throw new ApiError(403, "Students may only access their own assessment data");
    }
